...
<body class="bg-gray-200">
    ...
    <div class="container mx-auto  p-4">
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-3xl font-bold text-red-600">All Products</h1>
            <div>
                <button onclick="document.getElementById('addProductModal').style.display='block'" class="bg-red-500 text-white px-4 py-2 rounded">Add Product</button>
                <a href="cart.php" ><button class="bg-gray-500 text-white px-4 py-2 rounded">View Cart</button></a>
            </div>
        </div>
        <div class="grid grid-cols-3 gap-4">
            <?php
                ...
                while ($row = pg_fetch_assoc($result)) {
                    ...
                    echo '<img src="' . $row['image_url'] . '" alt="' . $row['name'] . '" class=" mb-4">';
                    echo '<h2 class="text-xl font-bold">' . $row['name'] . '</h2>';
                    echo '<p>' . $row['description'] . '</p>';
                    echo '<p class="text-lg font-semibold">$' . $row['price'] . '</p>';
                    echo '<form method="post" action="add_to_cart.php">';
                    echo '<input type="hidden" name="product_id" value="' . $row['id'] . '">';
                    echo '<button type="submit" class="bg-purple-500 text-white px-4 py-2 rounded mt-2">Add to Cart</button>';
                    echo '</form>';
                    echo '</div>';
                }
                pg_close($conn);
